...
export default {
  ...
  methods: {
    ...
    loadData () {
      ...
      api.selectArrearsDetailStatistics(searchForm).then(({ data }) => {
    ...
    initData () {
      ...
      loggerD('statisticsDetailGroupByCustomer', this.statisticsDetailGroupByCustomer)
      if (this.opt === '客户' || !this.statisticsDetailGroupByCustomer) {
        this.columns[1].show = false
      } else {
        this.columns[1].show = true
      }
      this.cardName = '【' + this.$route.params.bodyName + '】' + this.opt + '欠费统计(' + this.searchForm.startDate + ' 至 ' + this.searchForm.endDate + ')'
      this.loadData()
    },
    exportData () {
      ...
      api.exportData(queryExcelBody).then(response => {
        if (response.status == 200) {
          fileDownload(response.data, '欠费详情统计_' + timestamp + '.xls')
        } else {
