...
export default {
  ...
  methods: {
    ...
    loadData () {
      ...
      api.selectPaymentDetailStatistics(searchForm).then(({ data }) => {
    ...
    initData () {
      ...
      this.cardName = '【' + this.$route.params.bodyName + '】' + this.opt + '收付统计(' + this.searchForm.startDate + ' 至 ' + this.searchForm.endDate + ')'
      this.loadData()
    },
    exportData () {
      ...
      api.exportData(queryExcelBody).then(response => {
        if (response.status == 200) {
          fileDownload(response.data, '收付详情统计_' + timestamp + '.xls')
        } else {
