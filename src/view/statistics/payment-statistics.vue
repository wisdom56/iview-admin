    ...
    <div>
        <menu-content-layout menuWidth="250" :menu-tab="false">
            <div slot="menu" class="pd-5">
                <statistics-search-info @on-search="loadData" :loading.sync="tableData.loading" type="payment"></statistics-search-info>
            </div>
            <div class="page-box" slot="content">
                <Card>
                    <Row class="mb-10">
                        <i-col span="12">
                            <menu-content-layout-title class="flex-none" style="width: 150px;">收付统计</menu-content-layout-title>
                        </i-col>
                        <i-col span="5">
...
export default {
  ...
  methods: {
    ...
    onDblClick (row) {
      ...
      this.$router.push({
        name: 'payment-detail-statistics',
        params: {
    ...
    loadData (searchForm, opt) {
      ...
      api.selectPaymentStatistics(searchForm).then(({ data }) => {
    ...
    exportData () {
      ...
      let queryExcelBody = {
        ...
        title: this.opt + '收付统计(' + this.searchForm.startDate + ' 至 ' + this.searchForm.endDate + ')',
        sortList: this.tableData.sortList
      }
      const timestamp = new Date().getTime()
      api.exportData(queryExcelBody).then(response => {
        if (response.status == 200) {
          fileDownload(response.data, '收付统计_' + timestamp + '.xls')
        } else {
