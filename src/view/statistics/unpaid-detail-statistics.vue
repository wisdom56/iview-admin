...
export default {
  ...
  methods: {
    ...
    loadData () {
      ...
      api.selectUnpaidDetailStatistics(searchForm).then(({ data }) => {
    ...
    initData () {
      ...
      this.cardName = '【' + this.$route.params.bodyName + '】' + this.opt + '未付统计(' + this.searchForm.startDate + ' 至 ' + this.searchForm.endDate + ')'
      this.loadData()
    },
    exportData () {
      ...
      api.exportData(queryExcelBody).then(response => {
        if (response.status == 200) {
          fileDownload(response.data, '未付详情统计_' + timestamp + '.xls')
        } else {
