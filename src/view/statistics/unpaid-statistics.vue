    ...
    <div>
        <menu-content-layout menuWidth="250" :menu-tab="false">
            <div slot="menu" class="pd-5">
                <statistics-search-info @on-search="loadData" :loading.sync="tableData.loading" type="unpaid"></statistics-search-info>
            </div>
            <div class="page-box" slot="content">
                <Card>
                    <Row class="mb-10">
                        <i-col span="12">
                            <menu-content-layout-title class="flex-none" style="width: 150px;">未付统计</menu-content-layout-title>
                        </i-col>
                        <i-col span="5">
...
export default {
  ...
  methods: {
    ...
    loadData (searchForm, opt) {
      ...
      api.selectUnpaidStatistics(searchForm).then(({ data }) => {
        this.tableData.loading = false
        if (data.returnCode === 200) {
          let moneyUnpaidSum = 0
          data.returnData.forEach(item => {
            if (item.moneyUnpaid) {
              moneyUnpaidSum = (parseFloat(moneyUnpaidSum) + parseFloat(item.moneyUnpaid)).toFixed(2)
            }
          })
          data.returnData.forEach(item => {
            if (item.moneyUnpaid && moneyUnpaidSum != 0) {
              item.ratio = (parseFloat(item.moneyUnpaid / moneyUnpaidSum) * 100).toFixed(2)
            }
          })
    ...
    exportData () {
      ...
      let queryExcelBody = {
        ...
        title: this.opt + '未付统计(' + this.searchForm.startDate + ' 至 ' + this.searchForm.endDate + ')',
        sortList: this.tableData.sortList
      }
      const timestamp = new Date().getTime()
      api.exportData(queryExcelBody).then(response => {
        if (response.status == 200) {
          fileDownload(response.data, '未付统计_' + timestamp + '.xls')
        } else {
          ...
          sums[index] = values.reduce((prev, curr) => {
            ...
            if (!isNaN(value)) {
              ...
              if (columnsObj[column.property].type !== undefined && columnsObj[column.property].type === 'numCommas') {
                ...
                return this.$util.formatNumberCommas((parseFloat(prevNew) + parseFloat(currNew)).toFixed(column.precision))
